...
class mod_for_ren extends StatefulWidget {
  ...
  mod_for_ren(
      ...
      : super(key: key);
  mod_for_renState createState() => mod_for_renState();
}

class mod_for_renState extends State<mod_for_ren> {
  ...
  Future<String> _addRDV() async {
    final formData = _formKey.currentState!.value;

    try {
      if (formData['selectedOptionn'] == 'rappel personalisé') {
        await FirebaseFirestore.instance.doc(widget.docid).update({
          'nomMedecin': nomMedecin,
          'date': getdate(),
          'time':
              '${time.hour.toString().padLeft(2, '0')}:${time.minute.toString().padLeft(2, '0')}',
          'rappelPersonnalisé': getDuree(),
        });
      } else {
        await FirebaseFirestore.instance.doc(widget.docid).update({
          'nomMedecin': nomMedecin,
          ...
          'time':
              '${time.hour.toString().padLeft(2, '0')}:${time.minute.toString().padLeft(2, '0')}',
          'rappelPersonnalisé': formData['selectedOptionn'],
        });
      }

      // Récupère l'ID du document nouvellement créé
                  ...
                  children: [
                    ...
                    Expanded(
                      child: FormBuilderTextField(
                        initialValue: widget.list != null
                            ? widget.list["nomMedecin"]
                            : "",
                        name: 'nomMedecin',
